#!/usr/bin/env bash
# Table S8.1 (supplement): per-dataset MAE of every method
# Table S8.1（补充材料）：各方法逐数据集 MAE
# Usage:  [JOBS=<parallel processes>] [GPUS=<ids, e.g. 0,1>] [PYTHON=python] bash scripts/table_S8_1.sh
# 用法：  [JOBS=<并行进程数>] [GPUS=<GPU 编号，如 0,1>] [PYTHON=python] bash scripts/table_S8_1.sh
