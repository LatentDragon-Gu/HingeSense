#!/usr/bin/env bash
# Table S8.3 (supplement): per-dataset ablation results
# Table S8.3（补充材料）：逐数据集消融结果
# Usage:  [JOBS=<parallel processes>] [GPUS=<ids, e.g. 0,1>] [PYTHON=python] bash scripts/table_S8_3.sh
# 用法：  [JOBS=<并行进程数>] [GPUS=<GPU 编号，如 0,1>] [PYTHON=python] bash scripts/table_S8_3.sh
...
$PYTHON make_tables.py --only Table_S8_3
